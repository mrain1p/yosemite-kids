...
class MainActivity : androidx.fragment.app.FragmentActivity() {
    ...
    private fun handlePairIntent(intent: Intent?) {
        ...
        if (data.scheme != "yosemitekids" || data.host != "pair") return
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val screener = io.yosemitekids.app.data.Screener(io.yosemitekids.app.data.ScreeningStore(applicationContext)).also {
        ...
        RemotePlayerControl.playHandler = handler@{ req ->
            val videoId = io.yosemitekids.app.data.SponsorBlock.videoIdOf(req.url)
        ...
        lifecycleScope.launch { io.yosemitekids.app.data.Updater(applicationContext).autoCheck() }
        // The channel-index crawl runs whether or not the app is open — the
        // worker itself no-ops on any device that isn't the master.
        io.yosemitekids.app.data.IndexCrawlWorker.schedule(applicationContext)
        ...
        io.yosemitekids.app.data.ConfigSyncWorker.schedule(applicationContext)
        ...
        io.yosemitekids.app.data.ContentWarmWorker.schedule(applicationContext)
        setContent {
                ...
                fun assignedId(f: io.yosemitekids.app.data.Whitelist): String? =
                    f.deviceProfiles[myToken]?.takeIf { pid -> f.profile(pid) != null }

                // null with 2+ kids = the who's-watching screen must ask.
                fun resolveActive(f: io.yosemitekids.app.data.Whitelist): String? = when {
                ...
                // no kid (picker, pre-profile install) is plain Yosemite Kids teal.
                ...
                var kidTheme by remember(activeProfileId) {
                    mutableStateOf(
                        io.yosemitekids.app.data.KidPrefs(
                ...
                val vm: MainViewModel = viewModel(key = "main-${activeProfileId ?: "solo"}") {
                    MainViewModel(
                        ...
                        io.yosemitekids.app.data.ChannelPlaylistsCache(applicationContext),
                        ...
                        syncNotices = if (pairingStore.role() != PairingStore.Role.KID) {
                            io.yosemitekids.app.data.SyncNotices(appContext)
                        } else null,
                        ...
                        cacheStats = { device ->
                            LanClient.stats(device)?.let {
                                ...
                                withContext(Dispatchers.IO) {
                                    io.yosemitekids.app.data.StatsCache(appContext).save(device.key, it)
                                }
                            }
                        },
                        ...
                        LaunchedEffect(flow) {
                            ...
                            when (LanClient.requestPairing(flow.host, flow.port, myName, myToken)) {
                                ...
                                "pending" -> {
                                    ...
                                    status = "Open Yosemite Kids settings on the family's " +
                                    ...
                                    withContext(Dispatchers.IO) {
                                        ...
                                        val administered = deviceIsTv ||
                                            ...
                                            (pairingStore.paired().isEmpty() &&
                                                pairingStore.approvedPhones().isNotEmpty())
                                        if (administered) {
                                            io.yosemitekids.app.data.ProfileLooks(appContext)
