...
| `home.row` | one row of a kid's home screen — which shelf, on whose home (or the family's), at what `rank`, on or off — the pinned hero's shape one field along: sorted by `(rank, key)`, tombstoned with its kid, nothing written and nothing hashed while empty (`HomeRowsConfigTest`). A row naming a shelf this build has not got is carried and not drawn (`homeSections` reconciles at render). Every edit on either face goes through `HomeRows.withOrder` — the only place a rank is minted, an unknown shelf refused, and "the default, everything on" turned back into no rows (guard 70); the hub re-mints an incoming browser patch through it. Both faces read the list through `Whitelist.homeRowsFor` | ABSENT |
