---
name: pickwick-check
description: Build the Pickwick Android app and run its offline test suite (Kotlin unit tests + Cloudflare Worker tests). Use before committing, after any Kotlin/worker change, or when asked "does it build / do the tests pass".
---

# Pickwick check

Run from the repo root (`pickwick/`). Windows PowerShell:
...
2. `gradlew :app:testDebugUnitTest` for every test except
   `io.pickwick.app.ExtractorSmokeTest` (that one hits live YouTube and belongs
...
- A test failure names the class; open `app/src/test/java/io/pickwick/app/<Class>.kt`.
