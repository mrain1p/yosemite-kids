---
name: yosemite-kids-emulator
description: Boot the Android emulator, install the debug Yosemite Kids APK, seed a sample family config, launch, drive with adb input, and take screenshots. Use whenever a change needs to be seen or exercised without sideloading to a real device.
---

# Yosemite Kids on the emulator
...
.\scripts\emu.ps1 logcat        # last 200 Yosemite Kids log lines
...
up; set `$env:YOSEMITE_KIDS_SERIAL="emulator-5556"` and use the same verbs
...
- **The Android TV launcher steals the foreground.** Right after boot (and
  ...
  Yosemite Kids. Before any key sequence check
  `adb -s emulator-5556 shell dumpsys window | grep mCurrentFocus` contains
  `io.yosemitekids.app`; if not, `KEYCODE_HOME`, dismiss the promo, `launch` again.
...
Playback and fetch events are `YosemiteKids:` log lines (`chunked clen` = the
stream started, `open <id> failed`, `playlist shelves for <id>: n/m filled`,
`open channel by name`). An `ActivityManager: ANR in io.yosemitekids.app` with
