...
`GET /media` is the odd one out in a second way: it is the only route on this
box that sends bytes it fetched from somewhere else, and the only one whose
reply is not JSON. It exists because a browser cannot be handed a googlevideo
URL — see its row for the two separate reasons — and it is therefore the one
route with a concurrency cap and an executor of its own.
...
And the third, the four routes that are not a config patch at all:
