...
The kid listener answers **exactly five paths and 404s everything else**. That
...
| `GET /whoami` | claim cookie | — | `{"kid": "<id>", "name": "<name>", "video": "<id>"}` / `401 {"error":"claim"}` | The page's first call, and what it renders the code box from when the answer is 401. **Fails closed**: a browser that has not claimed learns nothing at all — not a child's name, not whether this hub has any children. `video` is the placeholder page's one video and goes with it in step 4. |
...
| `GET /` | none | — | the kid page | The placeholder of step 4, and served to anyone who asks because it contains nothing: no name, no video, no configuration. Everything a child sees arrives afterwards from `/whoami`, behind the cookie. **Only `/`** — every other path on this origin is a JSON 404, including `/index.html`. Anything but GET is 405. |
...
any credential is checked, so it reads the way `LanServer` does: a fixed pair
of worker threads, the same ten-second request timeout
(`HubServer.applyRequestTimeout`, on the companion so both listeners get it
whichever starts first), a 4 KB body cap, and a media pool that is separate
again.
