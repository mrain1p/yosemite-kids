# Fork notes, before 1.1.0

The first rounds of this fork, moved out of `docs/FORK-NOTES.md` on 2026-09-17
because the live changelog had grown past two thousand lines and the half
nobody reads was crowding the half they do. Nothing here is current: it is
what the player, the settings, the sync and the pairing looked like as they
were first built, and the reasons those shapes were chosen. The live file
carries 1.1.0 onward, and `docs/ROADMAP.md` is the only forward-looking doc.
...
- **The settings form adopts what it saved (after 1.0.4).** A save returns
  ...
  sync — it now reads the hashes from the bytes it sent. Guard 15 keeps
