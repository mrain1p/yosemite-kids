...
1. **Upstream port, all of it.** Upstream `1ae4cf1` (settings hub, per-kid
   ...
   Upstream is checked at the start of every round (`/pickwick-upstream`).
...
| 3 | You tab (replaces Favorites): avatar header, rows for Favorites / Watch later / Up next / History / Downloads with See all | `PickwickScreen`, `YouScreen.kt` (new), `MainViewModel.youShelves` |
...
| 6 | Home feed + channel page filter chips New / Random / Popular | `PhoneHome`, `PickwickScreen` channel header, `HomeState.filterVideos` |
...
| 8 | Dividers between home sections; "New for you" row on channel pages; "New from <channel>" in the portrait player | `HomeScreens`, `PickwickScreen`, `PlayerActivity` |
