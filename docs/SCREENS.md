...
| Search | `Search` → `SearchResults(q)` | `recentSearches`, `searchScreening` | `SearchField`, `VideoGrid` | the search icon in any header (phone); the rail's Search stop (TV) — the field takes focus on arrival so the system keyboard, with the remote's mic, raises itself; per-term × forgets one recent search |
