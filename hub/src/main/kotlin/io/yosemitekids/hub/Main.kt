 ...
 *
 * One port. The parents' console is at `/`, the kid app at `/kid`, and the
 * wall between them is built from credentials rather than from a second
 * listener — see [HubKidServer] for what holds it up. A family has one
 * address to know, and a compose file has one line to publish.
 */
fun main() {
    ...
    // Said separately, because it is the one address a parent types on a
    // tablet, and the log is where they will look for it.
    println("Kids watch at http://<this box>:$bound${HubKidServer.KID_PATH} — sign in with a QR from the console")
    master.start()
    crawl.start()
