...
#     (a) The kid listener serves EXACTLY these thirteen paths. Not a floor: a
...
$kidRoutes = @(Get-Content $kidSrv |
    ...
    Sort-Object -Unique)
if (($kidRoutes -join " ") -ne "/ /channel /claim /home /kid-icon /kid-manifest.webmanifest /kid-tokens.css /media /progress /search /thumb /whoami /you") {
    Fail-Guard "the kid origin serves [$($kidRoutes -join ' ')] and guard 57 expects [/ /channel /claim /home /kid-icon /kid-manifest.webmanifest /kid-tokens.css /media /progress /search /thumb /whoami /you]. Adding one is a decision: it must fail closed to the code prompt (guard 60), get a row in docs/LAN-API.md's kid section, and be named here."
}
